...
instance Eq a => Eq (List a) where
  ...
  (==) (LNode v1 next1) (LNode v2 next2) = (v1 == v2) && (next1 == next2)
...
instance Price Milk where
  price (Milk a) = 15 * (fromIntegral a)
...
simplify (RationalNumber n d) =
  let commonFactor = gcd n d in
    case commonFactor == 1 of
      True -> if n < 0 && d < 0
              then RationalNumber (-n) (-d)
              else RationalNumber n d
      False ->
        simplify (RationalNumber (div n commonFactor) (div d commonFactor))
...
instance Num RationalNumber where
  ...
  signum ratNum = let (RationalNumber n d) = simplify ratNum in
                    if n < 0 || d < 0
                    then -1
                    else 1
