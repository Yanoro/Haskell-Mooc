...
fmap2 f x = fmap (\i -> fmap f i) x

fmap3 :: (Functor f, Functor g, Functor h) => (a -> b) -> f (g (h a)) -> f (g (h b))
fmap3 f x = fmap (\i -> fmap (\j -> fmap f j) i) x
...
instance Functor List where
  fmap _ Empty = Empty
  fmap f (LNode c rest)= LNode (f c) (fmap f rest)
...
instance Functor TwoList where
  ...
  fmap f (TwoNode l r rest) = TwoNode (f l) (f r) $ fmap f rest
...
count t x = foldr (\curr acc -> if curr == t then acc + 1 else acc) 0 x
...
inBoth l1 l2 = foldr (\x acc -> if elem x l2 && (not $ elem x acc)
                                then [x] ++ acc else acc)
                     [] (toList l1)
...
instance Foldable List where
  foldr _ z Empty = z
  foldr f z (LNode c rest) = f c (foldr f z rest)
