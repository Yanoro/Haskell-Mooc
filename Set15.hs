...
statements l1 l2 = concat $ liftA2 combine l1 l2
...
calculator op n = liftA2 apply (validOp op) (readMaybe n)
  where validOp op = if (op == "negate" || op == "double")
                     then Just op else Nothing
        apply op num = if op == "negate"
                       then negate num else num * 2
...
validateDiv x y = liftA2 div (pure x) (isSafe y)
  where isSafe b = check (b /= 0) "Division by zero!" b
...
onlyDigits x = length (filter isDigit x) == length x

validateAddress :: String -> String -> String -> Validation Address
validateAddress streetName streetNumber postCode =
  Address <$> validStreet <*> validStreetNum <*> validPostCode
  where validStreet = check (length streetName <= 20)
                      "Invalid street name" streetName
        validStreetNum = check (onlyDigits streetNumber)
                      "Invalid street number" streetNumber
        validPostCode= check (onlyDigits postCode &&
                              length postCode == 5)
...
twoPersons name1 age1 employed1 name2 age2 employed2 =
  liftA2 (combine) (Person <$> name1 <*> age1 <*> employed1)
...
isBool :: String -> Validation (Either Bool Int)
isBool str =
  case readMaybe str :: Maybe Bool of
    Nothing -> invalid "Not a Bool"
    Just a -> pure (Left a)

isInt :: String -> Validation (Either Bool Int)
isInt str =
...
boolOrInt str = isBool str <|> isInt str
...
normalizePhone str = liftA2 (\x y -> x) (checkSize str') (checkDigit str')
  where checkSize s = check (length s <= 10) "Too long" s
        checkDigit s = traverse (\x -> if isDigit x then pure x
                                       else invalid ("Invalid character: " ++ [x]))
                       s
...
parseExpression str =
  if length expr /= 3 then invalid ("Invalid expression: " ++ str)
  else validOp <*> validArg arg1 <*> validArg arg2
  where expr = words str
        arg1 = expr !! 0
        op = expr !! 1
        arg2 = expr !! 2
        assignOp x = if x == "+" then Plus else Minus
        validOp = check (op == "+" || op == "-")
                  ("Unknown operator: " ++ op) (assignOp op)
        validVar var = check ((isAlpha $ head var) && length var == 1)
                  ("Invalid variable: " ++  var) (Variable $ head var)
        validNum num = check (foldr ((&&) . isNumber) True num)
                       ("Invalid number: " ++ num)
                       (Number (read num :: Int))
        validArg arg = validNum arg <|> validVar arg
...
instance Applicative Priced where
  pure x = Priced 0 x
  liftA2 f (Priced price1 val1) (Priced price2 val2) =
    Priced (price1 + price2) (f val1 val2)
...
myFmap f x = myLiftA2 (\fx y -> fx y) (myPure f) x
...
instance (Functor f, Functor g) => Functor (Both f g) where
  fmap fn (Both fa) = Both (fmap (\g -> fmap fn g) fa)
...
--  liftA2 :: Applicative f => (a -> b -> c) -> f a -> f b -> f c
    liftA2 f (Both a) (Both b) = Both $ liftA2 (\x y -> liftA2 f x y) a b
--  where hfmap (\g -> fmap fn g) fa
--
