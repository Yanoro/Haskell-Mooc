...
showDB :: Connection -> IO [(String, Int)]
showDB db = query_ db getAllQuery
...
deposit conn accName amount = execute conn depositQuery (accName, amount)
...
balance conn accName = do
  vals <- query conn balanceQuery [accName] :: IO [[Int]]
  return $ sum $ concat vals
...
parseCommand ts
  | ts !! 0 == T.pack "balance" &&
    length ts == 2 =
    do acc <- maybeIndex 1 ts
       Just (Balance acc)
  | ts !! 0 == T.pack "withdraw" &&
    length ts == 3 =
    do acc <- maybeIndex 1 ts
       amount <- maybeIndex 2 ts
       parsedAmount <- parseInt amount
       Just (Withdraw acc parsedAmount)
  | ts !! 0 == T.pack "deposit" &&
    length ts == 3 =
    do acc <- maybeIndex 1 ts
       amount <- maybeIndex 2 ts
       parsedAmount <- parseInt amount
       Just (Deposit acc parsedAmount)
  | otherwise = Nothing
