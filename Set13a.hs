...
checkNumber (str1, str2) =
  if containsNum str1 || containsNum str2
  then Nothing
  else Just (str1, str2)
  where containsNum s = length (filter isDigit s) > 0
...
checkCapitals ((x:xs),(y:ys)) =
  if isAsciiUpper x && isAsciiUpper y
  then Just ((x:xs),(y:ys))
  else Nothing
...
selectSum xs ys = helper ys 0
  where helper [] acc = Just acc
        helper (i:is) acc =
          do val <- safeIndex xs i
             helper is (acc + val)
...
countAndLog pred (x:xs) =
  do let val = if pred x then 1 else 0
     if val == 1 then Logger [show x] 1 else Logger [] 0
     fmap (+val) $ countAndLog pred xs
...
balance accountName = BankOp bOp
  where bOp (Bank bank) =
...
paren c = StateT checker
  where checker (-1) = Identity ((), -1)
        checker n = Identity $
          if (c == '(') then ((), n+1) else ((), n - 1)
...
count x = StateT fetch
  where fetch oldState =
            case lookup x oldState of
              Nothing -> Identity ((), oldState ++ [(x, 1)])
              Just a -> Identity ((), inc oldState x)
        inc [] _ = []
        inc ((k, j):rst) t = if k == t then (k, j + 1):rst
                             else (k, j):(inc rst t)
...
occurrences lst = counter lst
  where counter [] = StateT (\newState ->
                                Identity (length newState, newState))
        counter (c:rst) =
          do count c
             counter rst
