...
sumSuccess measurements = helper measurements 0 False
  where helper [] _ False = Left "no data"
        helper [] acc True = Right acc
        helper((Left _):xs) acc success = helper xs acc success
        helper ((Right n):xs) acc _ = helper xs (acc + n) True
...
open attemptCode (Lock locked lockCode) =
  if attemptCode == lockCode
  then Lock False lockCode
  else Lock locked lockCode

-- lock closes a lock. If the lock is already closed, nothing happens.
lock :: Lock -> Lock
lock (Lock locked code) = Lock True code

-- changeCode changes the code of an open lock. If the lock is closed,
-- nothing happens.
changeCode :: String -> Lock -> Lock
changeCode newCode (Lock locked code) =
  if locked then Lock locked code else Lock locked newCode
...
instance Eq Text where
  (==) (Text x) (Text y) = (removeWhiteSpace x) == (removeWhiteSpace y)
    where notWhiteSpace x = not (x == ' ')
                            && not (x == '\n')
          removeWhiteSpace = filter notWhiteSpace
...
getIndex n lst = helper 0 lst
  where helper _ [] = -1
        helper index (x:xs) = if n == x
                              then index
                              else helper (index + 1) xs

permute :: Permutation -> [a] -> [a]
permute [] _ = []
permute _ [] = []
permute perms lst = helper 0
  where helper count =
          if count == length lst
          then []
          else let currPermIndex = getIndex count perms
                   currPermVal = lst !! currPermIndex in
                 currPermVal:(helper (count + 1))
