...
longest :: Ord a => [[a]] -> [a]
longest (x:xs) = helper (x:xs) x
  where helper [] biggest = biggest
        helper (y:ys) biggest
          | length y > length biggest = helper ys y
          | length y < length biggest = helper ys biggest
          | otherwise = if head y < head biggest
                        then helper ys y
                        else helper ys biggest
...
incrementKey _ [] = []
incrementKey key ((head, val):xs)
  | key == head = (head, val + 1):(incrementKey key xs)
  | otherwise = (head, val):(incrementKey key xs)
...
winner scores player1 player2 =
  let score1 = Map.findWithDefault 0 player1 scores
      score2 = Map.findWithDefault 0 player2 scores in
...
freqs ys = foldr (\x currMap -> Map.alter alterFun x currMap) Map.empty ys
  where alterFun (Nothing) = Just 1
        alterFun (Just val) = Just (val + 1)
...
transfer from to amount bank
  ...
  | (fromValue - amount < 0) || amount < 0 = bank
  | otherwise =
    Map.alter (\(Just x) -> Just(x + amount)) to bank2
    where fromValue = Map.findWithDefault 0 from bank
          bank2 = Map.alter (\(Just x) -> Just(x - amount)) from bank
...
maxIndex arr = helper (Data.Array.assocs arr) (head $ indices arr)
  where
        helper [] biggest = biggest
        helper ((i,val):xs) biggest =
          case val > (arr ! biggest) of
            True -> helper xs i
            False -> helper xs biggest
