...
visit maze place =
  do visitedPlaces <- get
     if elem place visitedPlaces
     then return ()
     else do modify ([place]++)
             case lookup place maze of
                Nothing -> return ()
                Just destinations -> forM_ destinations (visit maze)
...
path maze place1 place2 =
  let (_, reachablePlaces) = runState (visit maze place1) [] in
    elem place2 reachablePlaces
...
safeIndex :: Eq a => a -> [a] -> Maybe Int
safeIndex n lst = helper lst 0
  where helper [] _ = Nothing
        helper (x:xs) counter =
          if x == n then Just counter else helper xs (counter + 1)

findSum2 :: [Int] -> [Int] -> [(Int,Int,Int)]
findSum2 ks ns =
  do i <- ks
     j <- ks
     case safeIndex (i + j) ns of
       Nothing -> []
       Just n -> [(i, j, ns !! n)]
------------------------------------------------------------------------------
...
getNVals :: Int -> [Int] -> [[Int]]
getNVals 0 _ = []
getNVals n xs =
  do j <- xs
     [j]:getNVals (n - 1) (tail xs)
...
f2 acc x =
  do previousValues <- get
     if elem x previousValues then return acc
     else do modify (++[x])
             return (x + acc)
...
instance Functor SL where
  -- implement fmap
  fmap f sl = SL (\oldState ->
                    let (val, newState, log) = runSL sl oldState in
                      (f val, newState, log))
...
instance Monad SL where
  ...
  return x = SL (\s -> (x, s, []))
  sl >>= f = SL (\oldState ->
                   let (val, newState, log) = runSL sl oldState
                       (val2, newState2, log2) = runSL (f val) newState in
                     (val2, newState2, log ++ log2))
...
mkCounter =
  do counter <- newIORef 0
     let incCounter = modifyIORef counter (+1)
         getCounter = readIORef counter
