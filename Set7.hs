...
velocity (Distance ds) (Time dt) = Velocity (ds / dt)

-- travel computes a distance given a velocity and a time
travel :: Velocity -> Time -> Distance
travel (Velocity v) (Time dt) = Distance (v * dt)
...
unique :: Eq a => [a] -> [a]
unique lst1 = helper lst1 []
  where helper [] curr = curr
        helper (x:xs) curr =
          let newElem = if elem x curr
                        then [] else [x] in
            helper xs (curr ++ newElem)
...
member val (Set (x:xs)) =
  if val == x
  then True
  else member val (Set xs)

-- add a member to a set
add :: (Ord a) => a -> Set a -> Set a
add a (Set b) = if elem a b then Set b else Set $ sort ([a] ++ b)
...
data State = Start | EggState | FlourState1 | SugarState1 | FlourState2 | SugarState2 | MixState | BakeState
           | Error | Finished
  deriving (Eq,Show)

step :: State -> Event -> State
step Start AddEggs = EggState
step EggState AddFlour = FlourState1
step FlourState1 AddSugar = SugarState2
step SugarState2 Mix = MixState
step EggState AddSugar = SugarState1
step SugarState1 AddFlour = FlourState2
step FlourState2 Mix = MixState
...
reverseNonEmpty (x :| []) = (x :| [])
reverseNonEmpty (x :| xs) = (last xs :| reverse (removeLast xs) ++ [x])
...
passwordAllowed password (MinimumLength minLen) = length password >= minLen
passwordAllowed password (ContainsSome chars) = helper chars
  where helper "" = False
        helper (c:cs) = if elem c password then True else helper cs
