...
rgb (Mix c1 c2) =
  let val1 = rgb c1
      val2 = rgb c2
      (r1, g1, b1) = (head val1, val1 !! 1, last val1)
      (r2, g2, b2) = (head val2, val2 !! 1, last val2) in
    [average [r1,r2], average [g1,g2], average [b1,b2]]
rgb (Invert c) =
  let val1 = rgb c
      (r, g, b) = (head val1, val1 !! 1, last val1) in
    [1 - r, 1 - g, 1 - b]
...
prettyPrint bin = reverse $ helper bin
  where helper End = ""
        helper (O x) = "0" ++ helper x
        helper (I x) = "1" ++ helper x

fromBin :: Bin -> Int
fromBin bin = helper bin 0
  where helper End _ = 0
        helper (O x) exponent = helper x (exponent + 1)
        helper (I x) exponent = (2^exponent) + helper x (exponent + 1)

toBin :: Int -> Bin
toBin 0 = O End
toBin n = reverseBin $ buildBin n $ largestExponent 0
  where buildBin _ (-1) =  End
        buildBin 0 exp = O $ buildBin 0 (exp - 1)
        buildBin x exp =
          case (x - (2^exp)) >= 0 of
            True -> I $ buildBin (x - (2^exp)) (exp - 1)
            False -> O $ buildBin x (exp - 1)
        largestExponent exp =
          case (n - (2^exp)) >= 0 of
            True -> largestExponent (exp + 1)
            False -> exp - 1

strToBin :: String -> Bin
strToBin str = helper $ reverse str
  where helper "" = End
        helper ('0':xs) = O (helper xs)
        helper ('1':xs) = I (helper xs)

reverseBin :: Bin -> Bin
reverseBin = strToBin . reverse . prettyPrint
