...
appendAll r [] = return ()
appendAll r strs = modifyIORef r (\x  -> x ++ concat strs)
...
hSelectLines h nums = do
  hLines <- hFetchLines h
  return $ map (\i -> hLines !! (i - 1)) nums
...
interact' f state = do
  line <- getLine
  let (continue, str, state') = f (line, state)
  putStrLn str
  if not continue then return state'
  else interact' f state'
