...
combine f (Picture p1) (Picture p2) = (Picture picFun)
  where picFun coord = let c1 = p1 coord
                           c2 = p2 coord in
                         f c1 c2
...
rectangle x0 y0 w h = Shape f
  where xInRect x =  x >= x0 && x <= (x0 + w - 1)
        yInRect y =  y >= y0 && y <= (y0 + h - 1)
        f (Coord x y) = xInRect x && yInRect y
------------------------------------------------------------------------------
...
instance Transform Zoom where
  apply (Zoom z) (Picture p) = Picture f
    where f coord = p $ zoomCoord z coord
...
instance (Transform a, Transform b) => Transform (Chain a b) where
  apply (Chain t1 t2) p = Picture f
    where f = let firstTransform = apply t2 p
                  (Picture secTransform) = apply t1 firstTransform in
                secTransform
...
instance Transform Blur where
  apply Blur (Picture p) = Picture f
    where colorsAverage :: [Color] -> Color
          colorsAverage xs =
            let (Color rS gS bS)= foldr (\(Color r1 g1 b1)
                                   (Color r2 g2 b2) ->
                                   (Color (r1 + r2)
                                          (g1 + g2)
                                          (b1 + b2)))
                              (Color 0 0 0) xs
                len = length xs in
              (Color (div rS len) (div gS len) (div bS len))
          f (Coord x y) =
            let cOrigin = p (Coord x y)
                cNorth = p (Coord x (y - 1))
                cEast = p (Coord (x + 1) y)
                cSouth = p (Coord x (y + 1))
                cWest = p (Coord (x - 1) y) in
              colorsAverage [cOrigin, cNorth, cEast, cSouth, cWest]
...
nApply :: (a -> a) -> Int -> a -> a
nApply f count starterValue = helper count starterValue
  where helper 0 acc = acc
        helper n acc = helper (n - 1) (f acc)

instance Transform BlurMany where
  apply (BlurMany n) p = nApply (apply Blur) n p
------------------------------------------------------------------------------
