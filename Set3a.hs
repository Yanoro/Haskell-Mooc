...
firstHalf s = take middleLength s
        where middleLength = if (((length s) `mod` 2) == 0)
                             then length s `div` 2
...
joinToLength n lst = [str1 ++ str2 | str1 <- lst, str2 <- lst,
                       length (str1 ++ str2)== n]
...
multiApp f hs val = f $ helper hs val
  where helper [] x = []
...
interpreter commands = helper commands 0 0
  where helper [] x y = []
        helper (s:ss) x y
          | s == "up" = helper ss x (y + 1)
          | s == "down" = helper ss x (y - 1)
          | s == "left" = helper ss (x - 1) y
          | s == "right" = helper ss (x + 1) y
          | s == "printX" = show x : helper ss x y
          | s == "printY" = show y : helper ss x y
          | otherwise = helper ss x y
